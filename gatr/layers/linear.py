...
class EquiLinear(nn.Module):
    ...
    def __init__(
        ...
        # Scalars -> MV scalars
        self.s2mvs: Optional[nn.Linear]     # Map scalars to multivector space
        if in_s_channels:
    ...
    def forward(
        ...
        outputs_mv = equi_linear(multivectors, self.weight)  # (..., out_channels, 16) each basis matrix represents a specific equivariant linear mapping

        if self.bias is not None:
            bias = embed_scalar(self.bias) # Embed bias into multivector space: scalars + non_scalar
            outputs_mv = outputs_mv + bias

        if self.s2mvs is not None and scalars is not None:
            # Avoid inplace operations, use scatter_add or index_copy alternative
            s2mvs_out = self.s2mvs(scalars)
            # Create new tensor to store result
            new_outputs_mv = outputs_mv.clone()
            new_outputs_mv[..., 0] = outputs_mv[..., 0] + s2mvs_out
            outputs_mv = new_outputs_mv
