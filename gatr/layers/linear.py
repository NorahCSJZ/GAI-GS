...
class EquiLinear(nn.Module):
    ...
    def __init__(
        ...
        # Scalars -> MV scalars
        self.s2mvs: Optional[nn.Linear]     # 将标量映射到多向量空间中
        if in_s_channels:
    ...
    def forward(
        ...
        outputs_mv = equi_linear(multivectors, self.weight)  #  (..., out_channels, 16) 每个基矩阵代表一种特定的等变线性映射

        if self.bias is not None:
            bias = embed_scalar(self.bias) # 将偏置项嵌入到多向量空间中 scalars + non_scalar
            outputs_mv = outputs_mv + bias

        if self.s2mvs is not None and scalars is not None:
            # 避免inplace操作，使用scatter_add或index_copy的替代方案
